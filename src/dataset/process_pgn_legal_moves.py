    ...
    for node in game.mainline():
        ...
        if "[%eval" in comment:
            ...
            if len(eval_parts) > 1:
                ...
                data_dict = {"FEN": fen, "FEN_processed": fen_no_counts, "legal_moves": list(legal_moves), "legal_move_count": board.legal_moves.count()}
            ...
            fen = board.fen()
            fen_no_counts = ' '.join(fen.split(' ')[:-2])
            data_dict = {"FEN": fen, "FEN_processed": fen_no_counts, "cp": 0, "legal_moves": list(legal_moves), "legal_move_count": board.legal_moves.count()}
            results.append(data_dict)
            ...
            fen = board.fen()
            fen_no_counts = ' '.join(fen.split(' ')[:-2])
            data_dict = {"FEN": fen, "FEN_processed": fen_no_counts, "mate": mate, "legal_moves": list(legal_moves), "legal_move_count": board.legal_moves.count()}
            results.append(data_dict)
        else:
            break
    return results
...
if __name__ == '__main__':
    ...
    extract_fens_and_evals_to_jsonl(
        ...
        r"C:\Users\Ahmad-personal\PycharmProjects\chess_stackfish_evals\data\lichess_db_standard_rated_2024-04.pgn.zst",
        r"C:\Users\Ahmad-personal\PycharmProjects\chess_stackfish_evals\data\lichess_db_standard_rated_2024-04.jsonl",
    )
